...
class EpisodeReplay:
    def __init__(self, simulator_name, live_feeds):
        ...
        """
        self.simulator = simulator_factory.factory.create(simulator_name,
                                                          web_window_size=900)
    ...
    def set_episode_by_fn_and_idx(self, game_fn, task_idx, episode_idx):
        ...
        structured_log = Dataset.import_json(file_name=game_fn,
                                             process_init_state=False,
                                             version="2.0")
    ...
    def play_episode(
        ...
        """
        if not force_replay and obs_dir is not None and os.path.isdir(
                obs_dir) and len(os.listdir(obs_dir)) > 0:
            logger.warn(
                "play_episode skipping playback in non-empty dir '%s'" %
                obs_dir)
            return False, False

        api_success, init_state = self._set_up_new_episode(
        ...
        self._write_last_states_and_frames(init_state, obs_dir,
                                           target_object_active, write_frames,
                                           write_states)
        ...
        if shutdown_on_finish:
            ...
            logger.info(
                ...
                % (len(self.episode.interactions), int(api_success),
                   int(task_success)))
    ...
    #     if not force_replay and os.path.isdir(out_dir) and len(
    ...
    #     for t in tqdm.tqdm(
    #             sorted(timestamps_to_fns,
    #                    key=lambda x: float(x)
    #                    if x and x != "end" else float("inf"))):
    ...
    #                     (
    #                         frame_border +
    #                         ref_im.width * frame_layout[frame_type][0],
    #                         frame_border +
    #                         ref_im.height * frame_layout[frame_type][1],
    #                     ),
    #                 )
    ...
    #                         (
    #                             frame_border +
    #                             ref_im.width * frame_layout["text"][0] + 5,
    #                             frame_border +
    #                             ref_im.height * frame_layout["text"][1] +
    ...
    #                     line_idx += 1
    #         frame.save(os.path.join(out_dir, "%05d.jpeg" % frame_idx),
    #                    format="jpeg")
    #         frame_idx += 1
    #     logger.info("... done; wrote %d assembled frames" % frame_idx)

    #     if frame_idx > 0:
    #         cmd = ('ffmpeg -r 1 -start_number 0 -i "' + out_dir +
    ...
    def stitch_episode_video(self, obs_dir, font_fn, force_replay=False):
        ...
        Stitch together a video of the episode for demo/inspection purposes.
        obs_dir - the observations to read in
        force_replay: if False, skips playback if the [obs_dir]/video is non-empty.
        """
        out_dir = os.path.join(obs_dir, "video")
        if not force_replay and os.path.isdir(out_dir) and len(os.listdir(out_dir)) > 0:
            logger.warn("Video stitching skipped in non-empty directory '%s'" % out_dir)
            return
        ...
        for fn in frame_fns:
            if fn.split(".")[-1] == "jpeg":
                ref_im = Image.open(fn)
                frame = Image.new("RGB", (ref_im.width * 2, ref_im.height * 1))
                break
        ...
        for t in tqdm.tqdm(sorted(timestamps_to_fns, key=lambda x: float(x) if x and x != "end" else float("inf"))):
            for fn in timestamps_to_fns[t]:
                ...
                if frame_type in frame_layout:  # visual observation
                    im = Image.open(fn)
                    frame.paste(im, (ref_im.width * frame_layout[frame_type][0], ref_im.height * frame_layout[frame_type][1]))
                elif fn.split(".")[-1] == "json":  # text observation
                    with open(fn, "r") as f:
                        contents = json.load(f)
                    text_output_path = os.path.join(out_dir, f"{frame_type}_{t}.txt")
                    with open(text_output_path, "w") as text_file:
                        text_file.write(json.dumps(contents, indent=4))
        ...
        if frame_idx > 0:
            cmd = ('ffmpeg -r 1 -start_number 0 -i "' + out_dir + '/%05d.jpeg" -c:v libx264 -vf "fps=25,format=yuv420p" ' + os.path.join(out_dir, "video.mp4"))
            logger.info("Executing: " + cmd)
            os.system(cmd)
            logger.info("... done")
        else:
            logger.warn("No frames extracted to stitch video for %s" % out_dir)

    def write_cur_state(self, frame_idx, obs_dir, init_state):
        cur_state = reduce_float_precision(
    ...
    def write_frames(self, frame_idx, obs_dir, target_object_active):
        ...
        self._write_frame(
            frames["targetobject"]
            if target_object_active else np.zeros_like(frames["targetobject"]),
            os.path.join(
                obs_dir,
    ...
    def _play_single_interaction(
        ...
        logger.debug("taking action <<%s, %s>>" %
                     (action_definition["action_type"],
                      action_definition["action_name"]))
        logged_success = self.episode.interactions[idx].action.success
        interact_oid = (self.episode.interactions[idx].action.oid
                        if action_definition["action_type"]
                        == "ObjectInteraction" else None)
        if logged_success == 1:
    ...
    def _add_interaction(self, idx, interact_oid, logged_success):
        ...
        if interact_oid is not None and (
            (logged_success
             and not self.episode.interactions[idx].action.success)
            ...
            mask_frame = self.simulator.get_target_object_seg_mask(
                interact_oid)["mask"]
            mask_points = np.where(mask_frame == 1)[:2]
            if len(
                    mask_points[0]
            ) > 0:  # if any part of the object is visible in the frame, pick a point on it
                rpoint_idx = np.random.randint(0, len(mask_points[0]))
                override_interaction = copy.deepcopy(
                    self.episode.interactions[idx])
                override_interaction.action.x = mask_points[1][
                    rpoint_idx] / self.simulator.web_window_size
                override_interaction.action.y = mask_points[0][
                    rpoint_idx] / self.simulator.web_window_size
                logger.info("... override interaction %s" %
                            override_interaction)  # DEBUG
                self.simulator.add_interaction(override_interaction)
                if self.episode.interactions[
                        idx].action.success != logged_success:
                    logger.info(
                ...
                logger.info("... override interaction with oid %s" %
                            interact_oid)
                cur_objs = self.simulator.get_objects()
                logger.info("Cur objs (ID, visible): %s " %
                            str([(obj["objectId"], obj["visible"])
                                 for obj in cur_objs]))
                self.simulator.add_interaction(self.episode.interactions[idx],
                                               on_oid=interact_oid,
                                               force=True)
    ...
    def _set_up_new_episode(self, obs_dir, turn_on_lights, task=None):
        ...
        self.simulator.start_new_episode(
            ...
            if self.episode.commander_embodied == "True" else False,
        )
        logger.info("... done")

        logger.info("Loading initial scene state...")
        _, s = self.simulator.load_scene_state(
            init_state=self.episode.initial_state)
        ...
        if task is not None:
            ...
            logger.info("Setting task %s with task_params %s..." %
                        (self.task, self.task_params))
            self.simulator.set_task_by_name(task_name=self.task,
    ...
    def _write_last_states_and_frames(self, init_state, obs_dir,
                                      target_object_active, write_frames,
                                      write_states):
        frame_idx = "end"
        if obs_dir is not None and write_states:
    ...
    def _write_progress_check(self, idx, obs_dir, action_definition,
                              target_object_active):
        frames = self.simulator.get_latest_images()
            ...
            json.dump(
                {
                    ...
                    "utterance":
                    self.episode.interactions[idx].action.utterance,
                },
                f,
            )
    ...
    def _turn_on_lights(self):
        logger.warn(
            "Turning on lights... This should not be used for experiments")
        objects = self.simulator.get_objects()
        light_switches = [
            obj for obj in objects if "LightSwitch" in obj["objectType"]
        ]
